...
def parser():
    ...
    infer_parser.add_argument('--batch_size', default=32, type=int,
                              help='batch size')
    infer_parser.add_argument('--input_channels', default=108, type=int,
    ...
    infer_parser.add_argument('--vertical_trim', type=int, default=20,
                              help='how many rows to remove from the spectrogram ')
    infer_parser.add_argument('--n_fft', type=int, default=1150,
    ...
    tunable = train_parser.add_argument_group(title='tunable args', description='arguments in this group are'
                                                                                ' tunable')
    tunable.add_argument('--n_fft', type=int, default=1150,
                         help='number of ffts used to create the spectrogram')
    tunable.add_argument('--learning_rate', type=float, default=0.00040775,
                         help='initial learning rate')
    tunable.add_argument('--vertical_trim', type=int, default=20,
                         help='how many rows to remove from the low-frequency range of the spectrogram.')
    tunable.add_argument('--mask_beginning_and_end', type=int,
                         default=1,
                         help='whether or not to mask the beginning and end of single-label chirps')
    tunable.add_argument('--begin_mask', type=int, default=28,
    ...
    non_tunable.add_argument('--mel', action='store_true', help='whether or not the data was created using'
                                                                'a mel spectrogram')
    ...
    non_tunable.add_argument('--num_nodes', type=int, default=1, help='number of nodes. If you want to train with 8'
                                                                      ' GPUs, --gpus should be 4 and --num_nodes'
                                                                      ' should be 2 (assuming you have 4 GPUs per '
                                                                      ' node')
    non_tunable.add_argument('--epochs', type=int, default=300, help='max number of epochs to train')
    non_tunable.add_argument('--check_val_every_n_epoch', type=int,
    ...
    non_tunable.add_argument('--num_workers', type=int, default=32,
                             help='number of threads to use when loading data')

    # extract
    extract_parser = subparsers.add_parser("extract", add_help=True)
    extract_parser.add_argument('--no_mel_scale', action='store_false',
                                help='whether or not to calculate a mel spectrogram. Default:'
                                     ' calculate it.')
    extract_parser.add_argument('--n_fft', default=1150, type=int,
